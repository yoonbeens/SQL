...
SELECT TO_CHAR(sysdate, 'YYYY-MM-DD DY DAY AM PM HH:MI:SS') FROM dual;
...
SELECT TO_CHAR(20000.26, '99999.9') FROM dual;
...
FROM employees --1
WHERE (sysdate - hire_date) / 365 >= 17 --2
ORDER BY 근속년수 DESC; --4
...
SELECT
    ...
    employee_id,
    first_name,
    (CASE --case의 기준을 생략 후 밑에서 따로 지정 가능
